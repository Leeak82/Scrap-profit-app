...
const categoryTips = {
  general: ['Check sold listings, not just active listings', 'Bundle low-value items', 'Clean before photos'],
  ...
  collectibles: ['Do not clean aggressively', 'Look for maker marks and dates', 'Check completed sold comps'],
...
function listingText(item, category, fastPrice, fairPrice) {
  const cleanName = item || 'Storage unit find';
  return `${cleanName}\n\nPrice: $${fastPrice}\n\nGood storage-unit find. See photos for condition. I priced it below the higher online comps for a faster local sale.\n\nDetails:\n- Category: ${category}\n- Condition: used / as shown\n- Estimated fair value: around $${fairPrice}\n- Pickup/local meet preferred\n\nMessage with questions. First reasonable offer gets it.`;
}
...
function analyze() {
  ...
  els('result').innerHTML = `
    ...
    <div class="action">
      <div class="score ${scoreClass}">Profit Score: ${score}/100</div>
      <p>${recommendation(score, fastPrice, category)}</p>
    </div>
    <div class="action">
      <b>What makes it better:</b>
      <p>${tips.map(t => `• ${t}`).join('<br>')}</p>
    </div>
    <div class="action">
      <b>Marketplace post</b>
